...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      setNewAddress({...newAddress,value:"",isAddingNewAddress:false})
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
         const res = await axios.post(`${config.endpoint}/cart/checkout`,
    ...
    <>
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box display="flex" className="space" flexDirection="column">
     ...
     {addresses.all.map((element) => (
      ...
      <Box
       ...
       className="space"
       onClick={()=>{
         setAddresses({...addresses,selected:element._id})
       }}
       className={(addresses.selected===element.address)?
           "address-item selected":"address-item not-selected"}
        ...
        >
          <Typography my="1rem">
               ...
               <Button onClick={()=>deleteAddress(token, element._id)} startIcon={<DeleteIcon />}> Delete</Button>
            ...
            <Button
            onClick={async ()=>performCheckout(token, items, addresses)}
